...
export const ShareCardGenerator: React.FC<ShareCardGeneratorProps> = ({
  ...
  const generateCard = async () => {
    ...
    if (contestant.photo_url) {
      try {
        const img = await loadImage(contestant.photo_url);
        const photoSize = 280;
    ...
    ctx.fillText(contestant.vote_count.toLocaleString(), textX, cardY + 280);
    
    ctx.fillStyle = '#666';
    ctx.font = '500 32px system-ui, sans-serif';
    ctx.fillText('votes', textX + ctx.measureText(contestant.vote_count.toLocaleString()).width + 20, cardY + 280);
    ...
    if (contest.brand_logo_url) {
      try {
        const logo = await loadImage(contest.brand_logo_url);
        const logoHeight = 50;
        const logoWidth = (logo.width / logo.height) * logoHeight;
        ctx.drawImage(logo, canvas.width - cardX - logoWidth - 40, cardY + cardHeight - logoHeight - 40, logoWidth, logoHeight);
      } catch (e) {
