-- Organization payout PIN: 6-digit PIN required for payout requests.
-- PIN hashes are stored in a separate table with no client SELECT access.

CREATE TABLE IF NOT EXISTS public.organization_payout_pins (
  organization_id uuid PRIMARY KEY REFERENCES public.profiles(id) ON DELETE CASCADE,
  pin_hash text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE public.organization_payout_pins ENABLE ROW LEVEL SECURITY;

CREATE OR REPLACE FUNCTION public.is_valid_payout_pin(pin text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION public.get_payout_organization_id()
RETURNS uuid
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT tm.organization_id, tm.permissions
  INTO v_org_id, v_permissions
  ...
  IF COALESCE((v_permissions->>'can_manage_payouts')::boolean, false) THEN
    RETURN v_org_id;
  END IF;
...
REVOKE ALL ON FUNCTION public.get_payout_organization_id() FROM PUBLIC;
...
DROP POLICY IF EXISTS "Organizations can insert their own payouts" ON public.payouts;
